      ...
      <div className="mx-auto max-w-[1280px]">
              ...
              ? [
                  {
                    ...
                    onSelect: () => selectScope("organization"),
                  },
                ]
              : []),
            {
        ...
        {scope === "organization" ? (
          ...
          <div className="mt-6 rounded-md border border-border bg-card p-4 text-sm leading-6 text-muted-foreground">
            This feed includes actions you performed, changes where you are the
            subject, changes to your linked employee record, and sanitized
            policy assignment effects affecting you. It does not include another
            member&apos;s activity or organization-wide history.
          </div>
        )}
        ...
        <AuditResults
          ...
          onLimit={(limit) =>
            updateUrl({ limit: String(limit), page: undefined }, false)
          }
        />
      </div>
    </div>
...
function OrganizationFilters({
  ...
  return (
    <section
      ...
      aria-label="Organization audit filters"
    >
      <div className="grid gap-3 sm:grid-cols-2 xl:grid-cols-4">
        <label className="space-y-2 text-xs font-medium">
          ...
          <SelectControl
            ...
            value={domain}
            onChange={(event) =>
              setDomain(event.target.value as AuditDomain | "")
            }
          >
            <option value="">All activity</option>
            {domainOptions
              .filter(([value]) =>
                visibleAuditDomains(canViewPolicy).includes(value),
              )
              .map(([value, label]) => (
      ...
      <div className="mt-4 grid gap-4 lg:grid-cols-2">
        {canViewActors ? (
          <ActorPicker
            businessId={businessId}
            selectedId={filters.actorId}
            selectedName={searchParams.get("actorName") ?? undefined}
            onSelect={(id, name) => updateUrl({ actorId: id, actorName: name })}
          />
        ) : null}
        {canViewEmployees ? (
          <EmployeePicker
            ...
            selectedName={searchParams.get("employeeName") ?? undefined}
            onSelect={(id, name) =>
              updateUrl({ employeeId: id, employeeName: name })
            }
          />
        ) : null}
      </div>
    ...
    <div className="rounded-md border border-border p-3">
      <label className="text-xs font-medium">
        ...
        <SelectControl
          ...
          value={selectedId ?? ""}
          onChange={(event) => {
            const member = items.find((item) => item.id === event.target.value);
            onSelect(member?.id, member?.userId.name);
          }}
        >
...
function EmployeePicker({
  ...
  const [page, setPage] = useState(1);
  const query = useBusinessEmployeesQuery(businessId, {
    page,
    limit: 10,
    ...(search ? { search } : {}),
  });
  ...
  return (
    <div className="rounded-md border border-border p-3">
      ...
      >
        ...
        <Button className="mt-7" type="submit" variant="outline">
          Search
        </Button>
      </form>
      <label className="mt-3 block text-xs font-medium">
        ...
        <SelectControl
          ...
          onChange={(event) => {
            ...
            onSelect(employee?.id, employee?.fullName);
          }}
        >
    ...
    <section className="mt-6" aria-live="polite">
      {data?.items.length ? (
        ...
        <div className="rounded-md border border-dashed border-border bg-card p-10 text-center text-sm text-muted-foreground">
          {scope === "organization"
            ? "No organization audit events match these filters."
            : "No activity affecting you has been recorded yet."}
        </div>
      )}
      {data ? (
        <Pagination
