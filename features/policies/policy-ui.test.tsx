  ...
  desiredPolicies: [
    {
      ...
      conditionEvaluations: {
        "rule-1": [
          {
            condition: { field: "state", operator: "equals", value: "Lagos" },
            actualValue: "Abuja",
            matched: false,
          },
        ],
  ...
  categoryDecisions: [
    {
      ...
      name: "Benefits",
      cardinality: "ONE",
      winnerPolicyIds: ["policy-winning"],
      suppressedPolicyIds: ["policy-suppressed"],
    },
  ],
...
describe("policy UI states", () => {
  it("renders manual winners, failed conditions, suppression, and historical warning", () => {
    ...
    expect(html).toContain(
      "complete historical employee-attribute snapshot is not available",
    );
    expect(html).toContain("Benefits");
  });

  it("renders loading, empty, and retryable error states", () => {
    expect(renderToStaticMarkup(<PolicyLoading />)).toContain(
      "Loading policies",
    );
    expect(
    ...
    ).toContain("No policy categories");
    expect(
      renderToStaticMarkup(
        <PolicyError message="Backend unavailable" retry={() => undefined} />,
      ),
        ...
        items={[
          {
            ...
            reason: "Requested",
            metadata: { triggeredByUserId: "user-1" },
