...
export function PolicyLoading({ label = "Loading policies…" }: { label?: string }) {
  return <PolicyPageFrame><div className="space-y-4"><div className="flex items-center gap-2 text-muted-foreground"><Loader2 className="animate-spin" />{label}</div><div className="h-32 animate-pulse rounded-xl bg-muted" /><div className="h-64 animate-pulse rounded-xl bg-muted" /></div></PolicyPageFrame>;
}

export function PolicyError({ message, retry }: { message: string; retry: () => void }) {
  return <div className="rounded-xl border border-destructive/30 bg-destructive/5 p-6"><h2 className="font-semibold text-destructive">Unable to load policy data</h2><p className="mt-2 text-sm text-muted-foreground">{message}</p><Button className="mt-4" variant="outline" onClick={retry}>Retry</Button></div>;
}

export function PolicyEmpty({ title, detail }: { title: string; detail?: string }) {
  return <div className="rounded-xl border border-dashed border-border p-10 text-center"><h2 className="font-semibold">{title}</h2>{detail ? <p className="mt-2 text-sm text-muted-foreground">{detail}</p> : null}</div>;
}
...
function JsonDetails({ label, value }: { label: string; value: unknown }) {
  ...
  return <div><p className="mb-1 text-xs font-semibold uppercase tracking-wide text-muted-foreground">{label}</p><pre className="max-h-64 overflow-auto rounded-lg bg-muted p-3 text-xs">{JSON.stringify(value, null, 2)}</pre></div>;
}
