...
export function PolicyAuditTable({ items }: { items: PolicyAudit[] }) {
  ...
  if (!items.length)
    return <PolicyEmpty title="No policy history matches these filters." />;
  return (
      ...
      <table className="w-full min-w-[920px] text-left text-sm">
        <thead className="border-b border-border bg-muted/50 text-xs text-muted-foreground">
          <tr>
            ...
            <th className="p-4">Changed fields</th>
            <th className="p-4">
        ...
        <tbody className="divide-y divide-border">
          {items.map((item) => {
            const technicalId = item.correlationId || item.reconciliationRunId;
            const initiator = auditInitiatingUser(item.metadata);
              ...
              <tr key={item.id} className="align-top">
                ...
                <td className="p-4">
                  ...
                  <p className="text-xs text-muted-foreground">
                    {item.actorUserId ||
                      initiator ||
                      item.actorBusinessMemberId ||
                      "Not available"}
                  </p>
                </td>
                <td className="p-4 text-xs text-muted-foreground">
                  {[
                    item.employeeId && `Employee ${item.employeeId}`,
                    item.policyId && `Policy ${item.policyId}`,
                    item.categoryId && `Category ${item.categoryId}`,
                  ]
                    .filter(Boolean)
                    .map((value) => (
                      <p key={value as string}>{value}</p>
                    ))}
                </td>
                <td className="p-4">
                ...
                <td className="p-4">
                  <Button
                    ...
                    onClick={() => setExpanded(isOpen ? undefined : item.id)}
                  >
                    {isOpen ? <ChevronUp /> : <ChevronDown />}Details
                  </Button>
                  {isOpen ? (
                    <div className="mt-3 w-[520px] space-y-3">
                      <JsonDetails label="Before" value={item.before} />
                      <JsonDetails label="After" value={item.after} />
                      {technicalId ? (
                        <p className="text-xs text-muted-foreground">
                          Technical ID: {technicalId}
                        </p>
                      ) : null}
                    </div>
                  ) : null}
