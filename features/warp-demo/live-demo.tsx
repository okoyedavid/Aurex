...
function Loading({ label }: { label: string }) { return <div className="grid min-h-64 place-items-center text-sm text-muted-foreground"><span className="flex items-center gap-2"><LoaderCircle className="size-4 animate-spin" />{label}…</span></div>; }
function InlineError({ error }: { error: Error }) { return <div className="mt-6 rounded-md border border-destructive/25 bg-destructive/10 p-5 text-sm text-destructive"><AlertCircle className="mr-2 inline size-4" />{error.message}</div>; }
