...
export type AuditDomain = "business" | "member" | "employee" | "policy" | "security";
...
export type AuditItem = {
  ...
  occurredAt: string;
  domain: AuditDomain;
  auditType: "business" | "membership" | "employee" | "policy" | "personal" | "security";
  action: string;
  actor: { type: string; displayName: string } | null;
  subject: { type: string; displayName: string } | null;
  summary: string;
  changes?: Array<{ field: string; before: AuditPrimitive; after: AuditPrimitive }>;
  reason?: string;
};
...
function normalizeAuditError(error: unknown, personal: boolean): never {
  ...
  throw new BusinessApiError(500, {
    message: "Audit activity could not be loaded. Check your connection and try again.",
  });
}
